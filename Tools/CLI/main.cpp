...
struct CliOptions {
    ...
    QString project;
    QString scenario;
    QString environment = QStringLiteral("default");
    QString executionId;
    QString url;
    bool headless = true;
    bool verbose = false;
    QString outputFormat = QStringLiteral("text");  // text, json, html
};
...
CliOptions parseArgs(int argc, char* argv[]) {
    ...
    opts.command = QString::fromLocal8Bit(argv[1]);

    for (int i = 2; i < argc; ++i) {
        if (std::strcmp(argv[i], "--project") == 0 && i + 1 < argc) {
            opts.project = QString::fromLocal8Bit(argv[++i]);
        } else if (std::strcmp(argv[i], "--scenario") == 0 && i + 1 < argc) {
            opts.scenario = QString::fromLocal8Bit(argv[++i]);
        } else if (std::strcmp(argv[i], "--env") == 0 && i + 1 < argc) {
            opts.environment = QString::fromLocal8Bit(argv[++i]);
        } else if (std::strcmp(argv[i], "--execution") == 0 && i + 1 < argc) {
            opts.executionId = QString::fromLocal8Bit(argv[++i]);
        } else if (std::strcmp(argv[i], "--url") == 0 && i + 1 < argc) {
            opts.url = QString::fromLocal8Bit(argv[++i]);
            ...
            opts.outputFormat = QString::fromLocal8Bit(argv[++i]);
...
int runCommand(const CliOptions& opts) {
    std::cout << "[Sentra] Running scenario '" << qUtf8Printable(opts.scenario)
              << "' in project '" << qUtf8Printable(opts.project)
              << "' (env: " << qUtf8Printable(opts.environment) << ")" << std::endl;
...
int listCommand(const CliOptions& opts) {
    std::cout << "[Sentra] Scenarios in project '" << qUtf8Printable(opts.project) << "':" << std::endl;
    // TODO: List scenarios from storage
    return 0;
}

int reportCommand(const CliOptions& opts) {
    std::cout << "[Sentra] Report for execution '" << qUtf8Printable(opts.executionId) << "':" << std::endl;
    // TODO: Load and display execution report
    return 0;
}

int baselineCommand(const CliOptions& opts) {
    std::cout << "[Sentra] Approving baselines for execution '" << qUtf8Printable(opts.executionId) << "'" << std::endl;
    // TODO: Approve baseline changes
    return 0;
}

int exploreCommand(const CliOptions& opts) {
    std::cout << "[Sentra] Starting autonomous exploration of '" << qUtf8Printable(opts.url) << "'" << std::endl;
...
int main(int argc, char* argv[]) {
    ...
    if (opts.command == "run") {
        ...
        std::cerr << "[Sentra] Unknown command: " << qUtf8Printable(opts.command) << std::endl;
        printUsage();
        return 1;
    }
}
